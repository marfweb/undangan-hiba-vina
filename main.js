/* ==========================================================================
   ...
   countdown, gallery reveal, music player.
...
(function () {
  ...
  /* ------------------------------------------------------------------
     3b. "The Wedding Of Hiba & Vina ... Save The Date" text reveal —
  ...
  safe('scroll reveals', function () {
    ...
    revealGroup('.gallery__item', { from: { opacity: 0, y: 50 }, stagger: .1 });
    ...
    // Subtle "breathing" zoom. Keep this close to 1 — a high value
    // permanently crops the faces out of the portrait frame.
    gsap.utils.toArray('.couple__photo img').forEach(function (img, i) {
      gsap.to(img, { scale: 1.05, duration: 9 + i, ease: 'sine.inOut', yoyo: true, repeat: -1 });
    });
  ...
  safe('countdown', function () {
    var target = new Date('2026-10-04T08:00:00+07:00').getTime();
  ...
  safe('hover effects', function () {
    if (!hasGsap) return;
    document.querySelectorAll('.btn-open, .btn-solid, .btn-ghost').forEach(function (el) {
